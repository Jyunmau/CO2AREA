...
def do_cal():
    ...
    print('人口增长率模型参数，参考多项式w1*P+W2*G+W3*PG+b，输出权重向量为[w1 w2 w3]，偏置向量为[b]')
    print(model_Pt.coef_)
    print(model_Pt.intercept_)
    print('GDP增长率模型参数，参考多项式W1*G+W2*PG+b，输出权重向量为[w1 w2]，偏置向量为[b]')
    ...
    plt.plot(t, (1 + Pt_predict) * P_o[:-1], 'g-')
    plt.draw()
    plt.figure()
    plt.title("GDP")
    plt.scatter(t, (1 + Gt) * G_o[:-1], marker='o', c='r')
    plt.plot(t, (1 + Gt_predict) * G_o[:-1], 'g-')
    plt.draw()
    plt.show()
